...
export interface RelayWebSocket extends WebSocket {
	// A unique peer identifier provided by the peer on connection, used for directing messages.
	peerId: string
...
export class RelayServer {
	...
	handleConnection(ws: RelayWebSocket) {
		this.loggerService.info("connection", `peer '${ws.pid}' connected to relay '${ws.relayId}'`)
		...
		ws.on("close", async () => {
			this.loggerService.info("connection", `peer '${ws.pid}' disconnecting from relay '${ws.relayId}'`)
		})
	}
	...
	runConnectionCheck() {
		...
		this.#wss.clients.forEach((ws) => {
			...
			if (!relaySocket.isAlive) {
				this.loggerService.info("connection", `disconnecting peer '${relaySocket.pid}' from relay '${relaySocket.relayId}' due to failed connection check`)
